...
/**
 * @file QueryObject.cpp
 * @author yangtianxing01(com@baidu.com)
 * @date 2014/08/30 12:03:23
 * @brief 
 *  
 **/
...
namespace btest{
using ::com::baidu::wd::knowledge::ver_1_0_0::QueryParam;
using ::com::baidu::wd::knowledge::ver_1_0_0::QueryParamPtr;
QueryObject::QueryObject(){
    query = ::sofa::create<QueryParam>();}
bool QueryObject::load_from_file(const std::string &file_name){
    std::ifstream ifs(file_name.c_str());
    if(!ifs){return false;}
    std::string tmp_str;
    getline(ifs, tmp_str);
    query->set_da_extra(tmp_str);
    getline(ifs, tmp_str);
    query->set_da_query(tmp_str);
    getline(ifs, tmp_str);
    query->set_text_query(tmp_str);
    ifs.close();
    return true;}
QueryParamPtr& QueryObject::get_query_param(void){
    return query;}
} //namespace btest
} //namespace movie_plugin
...
namespace btest{
//判断当天是否属于周几到周几的星期范围
bool TestMoviePluginImp::is_date_in_week(const struct tm *ptm,const std::string& week1,const std::string& week2) {
    std::string week;
    if (ptm->tm_wday == 0) {
        week = "7";
    }else { 
...
bool TestMoviePluginImp::is_date_in_season(const std::string& date,const std::string& season_str) {
...
bool TestMoviePluginImp::is_date_in_period(const std::string &date,const std::string &start,const std::string &end) {
...
//判断是否在指定的节假日内
bool TestMoviePluginImp::is_date_in_holiday(const struct tm *ptm,const std::vector<std::string> &holidays) {
    char today [9];
...
//计算营业时间
std::string TestMoviePluginImp::compute_today_openinghours(const struct tm *ptm,::faci::graphsearch::Json &structured_json) {
...
//计算票价
std::string TestMoviePluginImp::compute_today_price(const struct tm *ptm, ::faci::graphsearch::Json &structured_json) {
...
void TestMoviePluginImp::compute_scene_pc(::faci::graphsearch::Json& scene_json) {
