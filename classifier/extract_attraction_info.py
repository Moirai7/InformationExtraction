...
def Proc(key,line):
		...
		info = {}
		check = True
		line[3]=line[3].lower().decode('utf-8').replace(u'\uff1a',u':').replace(u'国定假日',u'国家法定节假日').replace(u'国定节假日',u'国家法定节假日').replace(u'每周七天开放','').replace(u'下午',u'pm').replace(u'点',u':00').replace(u'am to ',u'').replace(u'正午',u'12:00 ').replace(u'无休息日',u'').replace(u'全天开放',u'00:00-24:00').replace(u' - ',u'-').replace(u'日落',u'17:30')
		if line[3].find(u'平日')!=-1 and line[3].find(u'周六')!=-1 and line[3].find(u'周日')!=-1:
			line[3]=line[3].replace(u'平日',u'周一到周五')
		if line[3].find(u'平时')!=-1 and line[3].find(u'周末')!=-1:
			line[3]=line[3].replace(u'平时',u'周一到周六')
		for o in outlist:
			if line[3].find(o) != -1:
				check = False
				break
		l=rt_speci_time5.sub(u'',line[3])
		...
		if check is False:
			st =SpecialTime(line[3],info['openingHours'])
			if len(st)!=0:
